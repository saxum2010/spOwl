...
abstract class modSPowlHelper {
	...
	static function getimgList($params) {
		...
		foreach ($files as $file) {
			$image				= modSPowlHelper::getImages($path.'/'.$file,$thumbwidth,$thumbheight,$thumbratio);
	...
	static function getImages($image, $thumbwidth=100, $thumbheight=100, $thumbratio) {	  
		...
		if (isset($image)) {
			...
			if (function_exists("gd_info")) {
				...
				if (!class_exists("spOwlThumbnail")) include_once('class.spOwlThumbnail.php');
